...
class Solution {
    ...
    vector<int> myPrices;
public:
    void trade(bool own, int price, bool cooldown, int profit, int day)
    {
        //cerr << "own=" << own << ", price=" << price << ", cooldown=" << cooldown
        //    << ", profit=" << profit << ", day=" << day << "\n";
        if (day >= myPrices.size())
        {
            res = max(profit, res);
            return;
        }
        ...
        {
            // 卖
            trade(false, 0, true, profit + myPrices[day] - price, day + 1);
            // 不卖
            trade(true, price, false, profit, day + 1);
        }
        else
        {
            if (cooldown)
            {
                // 冷静期，不买
                trade(false, 0, false, profit, day + 1);
            }
            else
            {
                // 买
                trade(true, myPrices[day], false, profit, day + 1);
                // 不买
                trade(false, 0, false, profit, day + 1);
            }
        }
    ...
    int maxProfit(vector<int>& prices) {
        ...
        myPrices = prices;
        // 买
        trade(true, prices[0], false, 0, 1);
        // 不买
        trade(false, 0, false, 0, 1);
        return res;
    }
};
